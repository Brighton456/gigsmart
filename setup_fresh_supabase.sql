...
-- Gift codes policies (service role manages; authenticated users read codes and
-- increment usage during redemption - required by the app's client-side redeemGiftCode)
CREATE POLICY user_select_gift_codes ON gift_codes
FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY user_use_gift_codes ON gift_codes
...
-- =============================================
-- POST-TESTING FIXES (ported from migrations 20260815000002/3)
-- =============================================

-- Withdrawal requests status view: referenced by the app (getWithdrawalRequests);
-- security_invoker keeps RLS on withdrawal_requests in force.
...
-- Seed one test gift code so redemption can be exercised on a fresh project.
INSERT INTO gift_codes (code, reward_type, income_wallet_amount, main_wallet_amount, max_uses, current_uses, is_active, description, created_by)
